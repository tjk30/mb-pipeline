...
Join to results.  This varies depending on the level of the name, so do multiple joins in ascending taxonomic order:
- By form, if present
- By variety, if present
- Then by subspecies, if present
- Then by species (always present)
...
# Form
taxonomy.f <- filter(taxonomy, !is.na(forma))
# Variety
taxonomy.var <- filter(taxonomy, !is.na(varietas) & is.na(forma))
# Subspecies
taxonomy.ssp <- filter(taxonomy, !is.na(subspecies) & is.na(varietas) & is.na(forma))

# Species
# Had some trouble here with entries that terminate both at the species 
# level and a subspecies level: For simplicity, just make a list of all species-level entries
# This will ultimately be used in a right join so okay if there are 
# some that won't be incorporated
taxonomy.spp <- 
     taxonomy %>% 
...
Now join each to name, accession, ASV sequence:
...
```{r}
any(!(taxtab.species$asv %in% taxtab$asv))
```

The dimensions are a bit different, but think this is because of excluding accession number?
...
all(rownames(lca) == rownames(taxtab.lca))

taxtab.lca[lca > 1] = NA
taxtab.lca <- 
     rownames_to_column(taxtab.lca, var = 'asv')
```
...
#                                  'miniseq',
#                                  '20211222',
#                                  '20220505_no_tax_assignment.fasta'))
```
...
ggplot(samdf, aes(x = reads, 
     ...
     geom_histogram(binwidth = 10000, boundary = 0, 
                    color = 'black') +
     scale_fill_manual(values = c('gray80', '#1A9850', '#D73027')) +
     labs(x = 'Read count', 
          y = 'Count',
          fill = 'Sample type',
          title = '[DATE] MiniSeq run') +
...
```{r}
# Merge taxa with shared names
# Going this route because distinct NAs can be preserved
taxtab <- 
...
```{r}
# Form
taxtab$forma[duplicated(taxtab$forma, incomparables = NA)]
```

```{r}
# Variety
taxtab$varietas[duplicated(taxtab$varietas, incomparables = NA)]
```

```{r}
# Subspecies
taxtab$subspecies[duplicated(taxtab$subspecies, incomparables = NA)]
```

```{r}
# Need to rework this code to handle cases when things are distinct
# at a taxonomic level lower than species

# Species
taxtab %>% 
     filter(if_all(subspecies:forma, is.na)) %>% 
     filter(duplicated(species, incomparables = NA))

taxtab$species[duplicated(taxtab$species, incomparables = NA)]

# TODO: Follow up on Brassica SVs-- are they distinct sets of taxa?
# One maps to many taxa, and the other two are separate
# Likely need to follow up, but merge for now
taxtab %>% 
     filter(species == 'Brassica oleracea')
...
* Contaminant detected (onion), looks suspicious, but not removed
* Reads with 0 counts retained
* Duplicated taxa merged
* Did not try read count threshold
